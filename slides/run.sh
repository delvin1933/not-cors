...
if [[ $action == "gen" || $action == "start" ]]; then
    ...
    podman container run --rm -v $(pwd):/documents  cds-bdx-docker.repo-nf.groupeonepoint.com/gop-asciidoctor:latest gop-reveal index.adoc
fi

if [[ $action == "start" ]]; then
    echo "Run presentation web server on http://localhost:8080"
    podman run --rm -p 8080:80 -v $(pwd):/usr/share/nginx/html --name docker-slides nginx
fi
